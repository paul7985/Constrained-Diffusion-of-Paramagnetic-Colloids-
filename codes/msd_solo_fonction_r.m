...
Interval = 15;
MaxDisp = 10;
Radii = linspace(0, 1000, 100);  % <-- à adapter
...
for f = 1:length(filenames)
    ...
    % Initialisation pour stocker les dernières valeurs du MSD
    last_msd_values = zeros(1, length(Radii));
    avg_neighbors_values = zeros(1, length(Radii));
    ...
    for r = 1:length(Radii)
        ...
        if ~isempty(valid_msd)
            last_msd_values(r) = valid_msd(end); % Dernière valeur non-NaN
        else
            last_msd_values(r) = NaN; % Si aucun MSD valide, utiliser NaN
        end
        
        avg_neighbors_values(r) = avg_neighbors_per_frame;
    end

    % Affichage de la dernière valeur du MSD en fonction du rayon
    figure; clf;
    yyaxis left;
    plot(Radii, last_msd_values, '-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'Dernière valeur MSD');
    ylabel('Dernière valeur du MSD (px²)');
    yyaxis right;
    plot(Radii, avg_neighbors_values, '--s', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'Voisins moyens par image');
    ylabel('Voisins moyens par image');
    title(sprintf('MSD et Voisins vs Rayon - Particule #%d', selected_idx));
    xlabel('Rayon (px)');
    legend('show');
end
...
function [corrected_traj, avg_speed, total_neighbors, avg_neighbors_per_frame] = ...
    correct_local_drift_individual_verbose(traj, AllTraj, Radius)
