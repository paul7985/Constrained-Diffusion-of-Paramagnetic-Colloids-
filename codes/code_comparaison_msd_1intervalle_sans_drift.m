...
% [filename3, pathname3] = uigetfile('*.csv', 'Select the third file Results.csv');
...
% msd_mean3 = compute_MSD(filename3, pathname3, Interval, MaxDisp);
...
% plot(time_axis, msd_mean3, '-d', 'LineWidth', 1.5, 'DisplayName', sprintf('MSD %s', filename3));;
legend;
xlabel('Time (s)');
ylabel('MSD');
...
function msd_mean = compute_MSD(filename, pathname, Interval, MaxDisp)
    ...
    for i = 1:nTraj
        ...
        dx = x(end) - x(t0); % Déplacement total en x
        dy = y(end) - y(t0); % Déplacement total en y
        dt = t(end) - t(t0); % Durée totale
    ...
    for i = 1:nTraj
        ...
        msd{i} = zeros(T - t0, 1);
        for j = 1:T - t0
            msd{i}(j) = (x_corrected(j+t0) - x_corrected(t0)).^2 + ...
                              (y_corrected(j+t0) - y_corrected(t0)).^2;
        end
    end
