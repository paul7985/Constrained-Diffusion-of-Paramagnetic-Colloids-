...
for f = 1:length(filenames)
    ...
    figure; clf;
    yyaxis left;
    errorbar(Radii, mean_msd, std_msd, '-o', 'LineWidth', 1.5, 'MarkerSize', 6, ...
        'DisplayName', 'MSD moyen');
    ylabel('MSD moyen (px²)');
    yyaxis right;
    errorbar(Radii, mean_neighbors, std_neighbors, '--s', 'LineWidth', 1.5, 'MarkerSize', 6, ...
        'DisplayName', 'Voisins moyens par image');
...
function [corrected_traj, avg_speed, total_neighbors, avg_neighbors_per_frame] = ...
    ...
    temporal_window = 3 * Interval; % ±3 frames en temps (ms)
    ...
    if Radius == 0
        ...
        for i = 1:n_points
            ...
            neighbors_idx = find(time_diff <= temporal_window & ...
                                sqrt((AllTraj(:, 1) - x_i).^2 + (AllTraj(:, 2) - y_i).^2) <= Radius);
    ...
    avg_neighbors_per_frame = total_neighbors / n_points;
end
