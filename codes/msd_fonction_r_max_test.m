...
Interval = 15;
MaxDisp = 10;
Radii = linspace(0, 575, 30);  % Test avec 5 rayons, à ajuster à 200 après validation
ImageWidth = 1920;  % Ajuster selon la taille de l'image en pixels
ImageHeight = 1200; % Ajuster selon la taille de l'image en pixels
PixelToMicron = 1 / 1.8; % Conversion: 1.8 px = 1 µm
...
for f = 1:length(filenames)
    ...
    % Extraction des trajectoires originales et calcul de Rmax
    original_trajectories = cell(NrOfTrajs, 1);
    Rmax_values = zeros(NrOfTrajs, 1);
    for n = 1:NrOfTrajs
        idx = AllTraj(:, 4) == n;
        original_trajectories{n} = AllTraj(idx, [1 2 3]);
        traj = original_trajectories{n};
        % Calcul de Rmax : distance minimale au bord
        min_x = min(traj(:, 1));
        min_y = min(traj(:, 2));
        max_x = max(traj(:, 1));
        max_y = max(traj(:, 2));
        Rmax_values(n) = min([min_x, min_y, ImageWidth - max_x, ImageHeight - max_y]);
    end
    ...
    particles_count = zeros(1, length(Radii)); % Pour compter les particules valides
    ...
    for n = 1:NrOfTrajs
        ...
        for r = 1:length(Radii)
            ...
            % Vérifier si le rayon est inférieur à Rmax
            if Radius < Rmax_values(n)
                ...
                if ~isempty(valid_msd)
                    ...
                    particles_count(r) = particles_count(r) + 1; % Incrémenter le compteur
                else
                ...
                all_last_msd_values(n, r) = NaN;
                all_avg_neighbors_values(n, r) = NaN;
            end
        end
    end
    ...
    % Conversion des rayons en µm pour l'affichage
    Radii_um = Radii * PixelToMicron;
    
    % Affichage des informations pertinentes dans la console
    fprintf('Résultats pour le fichier %s :\n', filename);
    fprintf('MSD moyen (dernier lag) pour Rayon = %.0f px (%.2f µm) : %.2f ± %.2f px²\n', ...
        Radii(end), Radii_um(end), mean_msd(end), std_msd(end));
    fprintf('Voisins moyens pour Rayon = %.0f px (%.2f µm) : %.2f ± %.2f\n\n', ...
        Radii(end), Radii_um(end), mean_neighbors(end), std_neighbors(end));
    ...
    particle_color = [0.4 0.4 0.4];  % Gris
    ...
    % Tracé voisins et particules avec zone d'erreur (yyaxis droit)
    yyaxis right;
    hold on;
    % Zone d'erreur pour voisins
    fill([Radii fliplr(Radii)], ...
    ...
    % Tracé du nombre de particules (pas d'écart-type car c'est un compte)
    plot(Radii, particles_count, '-^', 'Color', particle_color, 'LineWidth', 1.5, 'MarkerSize', 6);
    ylabel('Nombre (voisins ou particules)');

    % Titres et axes
    title(sprintf('MSD, Voisins et Nombre de Particules vs Rayon - %s', filename));
    xlabel('Rayon (px)');
    grid on;
    legend({'Zone erreur MSD', 'MSD moyen', 'Zone erreur voisins', 'Voisins moyens', 'Nombre de particules'}, 'Location', 'best');
